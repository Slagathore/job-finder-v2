 ...
 * records scan_history. Shared by the HTTP ingress and usable elsewhere.
 */
export function ingestJobs(raw: RawJob[]): { added: number; duplicates: number; skipped: number } {
  const db = getDb();
  const now = Date.now();
  ...
  const insertJob = db.prepare(`
    INSERT INTO jobs (source, url, company, title, description, location_raw, work_mode, first_seen, status)
    VALUES (@source, @url, @company, @title, @description, @location_raw, @work_mode, @first_seen, 'discovered')
  ...
  const tx = db.transaction((rows: RawJob[]) => {
    for (const j of rows) {
      ...
      const res = insertJob.run({
        ...
        work_mode: inferWorkMode(j.location ?? ''), first_seen: now,
      });
